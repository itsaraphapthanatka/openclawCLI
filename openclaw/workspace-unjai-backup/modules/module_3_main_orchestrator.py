...
class MainOrchestrator:
    ...
    def __init__(self, config: Optional[OrchestratorConfig] = None):
        ...
        logger.info(f"Base URL for clips: {self.base_url}")
    ...
    async def _search_knowledge(
        ...
        try:
            ...
            for result in search_results:
                # Build full clip URL with base URL
                clip_url = result.metadata.get("clip_url", "") if hasattr(result.metadata, "get") else ""
                if not clip_url and result.video_id:
                    clip_url = f"/static/clips/{result.video_id}.mp4"
                if clip_url and not clip_url.startswith("http"):
                    clip_url = f"{self.base_url}{clip_url}"
            ...
            # If high similarity video found, set video_url with full base URL
            if search_results and search_results[0].score >= 0.70:
                clip_url = search_results[0].metadata.get("clip_url", "") if hasattr(search_results[0].metadata, "get") else ""
                if not clip_url:
                    clip_url = f"/static/clips/{search_results[0].video_id}.mp4"
                if not clip_url.startswith("http"):
                    clip_url = f"{self.base_url}{clip_url}"
                context.video_url = clip_url
